...
public class Element extends RunOnFailureKeywordsAdapter {
	...
	public void currentFrameContains(String text, String logLevel) {
		if (!isTextPresent(text)) {
			...
			throw new ABTNonFatalException(
	...
	public void elementShouldBeEnabled(String locator) {
		if (!isEnabled(locator)) {
			throw new ABTNonFatalException(String.format("Element %s is disabled.", locator));
		}
	}
	...
	public void elementShouldBeDisabled(String locator) {
		if (isEnabled(locator)) {
			throw new ABTNonFatalException(String.format("Element %s is enabled.", locator));
		}
	}
	...
	public String getElementAttribute(String attributeLocator) {
		...
		if (elements.size() == 0) {
			throw new ABTNonFatalException(String.format("Element '%s' not found.", parts[0]));
		}
	...
	public String getInnerElementId(String locator, String matchid, int index) {
		...
		if (elements.size() == 0) {
			throw new ABTNonFatalException(String.format("get Inner element '%s' not found.", locator));
		}
	...
	public void mouseDown(String locator) {
		...
		if (elements.size() == 0) {
			throw new ABTNonFatalException(String.format("ERROR: Element %s not found.", locator));
		}
	...
	protected boolean isEnabled(String locator) {
		...
		if (!formElement.isFormElement(element)) {
			throw new ABTNonFatalException(String.format("ERROR: Element %s is not an input.", locator));
		}

...
import abtlibrary.utils.WebDriverCache.SessionIdAliasWebDriverTuple;
	...
	protected WebDriver createLocalWebDriver(String browserName, DesiredCapabilities desiredCapabilities) {
		if ("ff".equals(browserName) || "firefox".equals(browserName)) {
			...
			try {
				...
				throw new ABTLibraryFatalException("Creating " + browserName + " instance failed.", e);
			}
		} else if ("android".equals(browserName)) {
			try {
				return new SelendroidDriver(desiredCapabilities);
			} catch (Exception e) {
				throw new ABTLibraryFatalException(e);
			}
		}

		throw new ABTLibraryFatalException(browserName + " is not a supported browser.");
	}

...
import abtlibrary.keywords.selenium2library.FormElement;
import abtlibrary.keywords.selenium2library.JavaScript;
import abtlibrary.keywords.selenium2library.Logging;
import abtlibrary.keywords.selenium2library.RunOnFailure;
import abtlibrary.keywords.selenium2library.Screenshot;
 ...
 * </table>
 */
public class ABTLibrary extends AnnotationLibrary {

	/**
	 * The list of keyword patterns for the AnnotationLibrary
	 */
	public static final String KEYWORD_PATTERN = "abtlibrary/keywords/**/*.class";

	/**
	 * The javadoc to libdoc converter
	 */
	public static final Javadoc2Libdoc JAVADOC_2_LIBDOC = new Javadoc2Libdoc(ABTLibrary.class);
	...
	private static String loadRobotLibraryVersion() {
		try {
			return ResourceBundle.getBundle(ABTLibrary.class.getCanonicalName().replace(".", File.separator))
					.getString("version");
	...
	/**
	 * ABTLibrary can be imported with optional arguments.<br>
	 * <br>
	 ...
	 *            run on failure.
	 */
	public ABTLibrary(String timeout, String implicitWait, String keywordToRunOnFailure) {
		super();
	...
	/**
	 * Returns the currently active ABTLibrary instance.
	 * 
	 * @return the library instance
	 * @throws ScriptException - if error occurs in script
	 */
	public static ABTLibrary getLibraryInstance() throws ScriptException {
		ScriptEngine engine = new ScriptEngineManager().getEngineByName("python");
		engine.put("library", "ABTLibrary");
		engine.eval("from robot.libraries.BuiltIn import BuiltIn");
		engine.eval("instance = BuiltIn().get_library_instance(library)");
		return (ABTLibrary) engine.get("instance");
	}
